...
public final class SendCoinsActivity extends AbstractWalletActivity
{
	...
	private void handleIntent(final Intent intent)
	{
		...
		if ((Intent.ACTION_VIEW.equals(action) || NfcAdapter.ACTION_NDEF_DISCOVERED.equals(action)) && intentUri != null && "worldcoin".equals(scheme))
		{
			try
			{
				final WorldcoinURI worldcoinUri = new WorldcoinURI(null, intentUri.toString());
				address = worldcoinUri.getAddress().toString();
				addressLabel = worldcoinUri.getLabel();
				amount = worldcoinUri.getAmount();
			}

...
/**
 * <p>Provides a standard implementation of a worldcoin URI with support for the
 * following:</p>
 *
 * <ul>
 ...
 * @see <a href="https://en.worldcoin.it/wiki/BIP_0021">BIP 0021</a>
 */
public class WorldcoinURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(WorldcoinURI.class);
    ...
    public static final String LITECOIN_SCHEME = "worldcoin";
    ...
    /**
     * Constructs a new WorldcoinURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws WorldcoinURIParseException if the URI is not syntactically or semantically valid.
     */
    public WorldcoinURI(String uri) {
        this(null, uri);
    }
     ...
     * @throws WorldcoinURIParseException If the input fails Worldcoin URI syntax and semantic checks.
     */
    public WorldcoinURI(NetworkParameters params, String input) {
        // Basic validation
        ...
        try {
            ...
            throw new WorldcoinURIParseException("Bad URI syntax", e);
        }
        ...
        if (input.startsWith("worldcoin://")) {
            ...
            throw new WorldcoinURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?");
        if (addressSplitTokens.length == 0 || "".equals(addressSplitTokens[0])) {
            throw new WorldcoinURIParseException("Missing address");
        }
        ...
        if (addressSplitTokens.length == 1) {
            ...
            if (addressSplitTokens.length == 2) {
                ...
                throw new WorldcoinURIParseException("Too many question marks in URI '" + uri + "'");
            }
        }
    ...
    private void parseParameters(NetworkParameters params, String addressToken, String[] nameValuePairTokens) {
        // Attempt to parse the addressToken as a Worldcoin address for this network
        try {
            Address address = new Address(params, addressToken);
            putWithValidation(FIELD_ADDRESS, address);
        } catch (final AddressFormatException e) {
            throw new WorldcoinURIParseException("Bad address", e);
        }
        ...
        for (String nameValuePairToken : nameValuePairTokens) {
            ...
            if (tokens.length != 2 || "".equals(tokens[0])) {
                throw new WorldcoinURIParseException("Malformed Worldcoin URI - cannot parse name value pair '" +
                        nameValuePairToken + "'");
            }
    ...
    private void putWithValidation(String key, Object value) {
        if (parameterMap.containsKey(key)) {
            throw new WorldcoinURIParseException("'" + key + "' is duplicated, URI is invalid");
        } else {
    ...
    /**
     * @return The Worldcoin Address from the URI
     */
    ...
    public static String convertToWorldcoinURI(Address address, BigInteger amount, String label, String message) {
        return convertToWorldcoinURI(address.toString(), amount, label, message);
    }

    /**
     * Simple Worldcoin URI builder using known good fields.
     * 
     * @param address The Worldcoin address
     * @param amount The amount in nanocoins (decimal)
     * @param label A label
     * @param message A message
     * @return A String containing the Worldcoin URI
     */
    public static String convertToWorldcoinURI(String address, BigInteger amount, String label, String message) {
        ...
        if (amount != null) {
            ...
            builder.append(Utils.worldcoinValueToPlainString(amount));

 ...
 * the lightweight SPV mode, a {@link com.google.worldcoin.store.BoundedOverheadBlockStore} may be a good choice.</p>
 *
...
public abstract class AbstractBlockChain {
     ...
     *
     * Following this one down to the genesis block produces the story of the economy from the creation of Worldcoin
    ...
    private boolean add(Block block, Set<Sha256Hash> filteredTxHashList, List<Transaction> filteredTxn, boolean tryConnecting)
        ...
        try {
            ...
            // article here for more details: http://code.google.com/p/worldcoinj/wiki/SecurityModel
            try {
                block.verifyHeader();
        ...
        for (Transaction tx : transactions) {
            try {
                if (listener.isTransactionRelevant(tx)) {
                    if (clone)
                        tx = new Transaction(tx.params, tx.worldcoinSerialize());

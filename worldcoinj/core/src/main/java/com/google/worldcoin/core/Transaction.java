 ...
 * the minting of new coins. A Transaction object corresponds to the equivalent in the Worldcoin C++ implementation.</p>
 *
 * <p>Transactions are the fundamental atoms of Worldcoin and have many powerful features. Read
 ...
 *
 * <p>All Worldcoin transactions are at risk of being reversed, though the risk is much less than with traditional payment
...
public class Transaction extends ChildMessage implements Serializable {
    ...
    /**
     * Used by WorldcoinSerializer.  The serializer has to calculate a hash for checksumming so to
     ...
     * value is determined by a formula that all implementations of Worldcoin share. In 2011 the value of a coinbase
    ...
    public String toString(AbstractBlockChain chain) {
        ...
        for (TransactionOutput out : outputs) {
            ...
            try {
                ...
                s.append(" ");
                s.append(worldcoinValueToFriendlyString(out.getValue()));
                s.append(" BTC");
     ...
     * features of Worldcoin are developed.
     *
        ...
        try {
            ...
            if ((sigHashType & 0x1f) == (SigHash.NONE.ordinal() + 1)) {
                ...
                if (inputIndex >= this.outputs.size()) {
                    ...
                    // Worldcoin implementation. The reference client also contains a bug in handling this case:
    ...
    protected void worldcoinSerializeToStream(OutputStream stream) throws IOException {
        ...
        for (TransactionInput in : inputs)
            in.worldcoinSerialize(stream);
        stream.write(new VarInt(outputs.size()).encode());
        for (TransactionOutput out : outputs)
            out.worldcoinSerialize(stream);

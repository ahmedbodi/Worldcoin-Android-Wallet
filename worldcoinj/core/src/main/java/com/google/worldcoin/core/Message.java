...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using both the Worldcoin proprietary serialization
...
public abstract class Message implements Serializable {
     ...
     * @param msg Worldcoin protocol formatted byte array containing message content.
     * @param offset The location of the first msg byte within the array.
     * @param protocolVersion Worldcoin protocol version.
    ...
    private void selfCheck(byte[] msg, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = worldcoinSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom Worldcoin protocol.
    ...
    /**
     * Should only used by WorldcoinSerializer for cached checksum
     *
     * @return the checksum
     */
    byte[] getChecksum() {
    ...
    /**
     * Should only used by WorldcoinSerializer for caching checksum
     *
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeWorldcoinSerialize()}, which is safe to mutate.
    ...
    public byte[] worldcoinSerialize() {
        byte[] bytes = unsafeWorldcoinSerialize();
     ...
     */
    public byte[] unsafeWorldcoinSerialize() {
        ...
        try {
            worldcoinSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (parseRetain) {
            ...
            // which means another call to worldcoinSerialize is coming.  If we didn't recache then internal
     ...
     * @throws IOException
     */
    final public void worldcoinSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw bytes use worldcoinSerialize().
     */
    void worldcoinSerializeToStream(OutputStream stream) throws IOException {
        log.debug("Warning: {} class has not implemented worldcoinSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so WorldcoinSerializer can avoid 2 instanceof checks + a casting.
     *
     * @return
     */

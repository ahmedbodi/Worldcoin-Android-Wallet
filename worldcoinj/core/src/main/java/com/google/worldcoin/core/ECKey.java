 ...
 *
 * <p>ECKey also provides access to Worldcoin-Qt compatible text message signing, as accessible via the UI or JSON-RPC.
...
public class ECKey implements Serializable {
    ...
    static {
        // All clients must agree on the curve to use by agreement. Worldcoin uses secp256k1.
    ...
    /**
     * Construct an ECKey from an ASN.1 encoded private key. These are produced by OpenSSL and stored by the Worldcoin
    ...
    /**
     * Output this ECKey as an ASN.1 encoded private key, as understood by OpenSSL or used by the Worldcoin reference
     ...
     * how ECDSA signatures are represented when embedded in other data structures in the Worldcoin protocol. The raw
    ...
    public static class ECDSASignature {
         ...
         * of the type used by Worldcoin we have to encode them using DER encoding, which is just a way to pack the two
    ...
    /**
     * Signs the given hash and returns the R and S components as BigIntegers. In the Worldcoin protocol, they are
    ...
    /**
     * Signs the given hash and returns the R and S components as BigIntegers. In the Feathercoin protocol, they are
     * usually encoded using DER format, so you want {@link com.google.feathercoin.core.ECKey.ECDSASignature#encodeToDER()}
    ...
    /**
     * Signs a text message using the standard Worldcoin messaging signing format and returns the signature as a base64
     * encoded string.
     *
    ...
    /**
     * Given an arbitrary piece of text and a Worldcoin-format message signature encoded in base64, returns an ECKey
     * containing the public key that was used to sign it. This can then be compared to the expected public key to
     * determine if the signature was correct. These sorts of signatures are compatible with the Worldcoin-Qt/worldcoind
     * format generated by signmessage/verifymessage RPCs and GUI menu options. They are intended for humans to verify
     * their communications with each other, hence the base64 format and the fact that the input is text.
     *
     * @param message Some piece of human readable text.
     * @param signatureBase64 The Worldcoin-format message signature in base64
     ...
     *
     * Because it is a critical failure if the private keys cannot be decrypted successfully (resulting of loss of all worldcoins controlled
